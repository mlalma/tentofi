...
The penalty margin does not need to be equal to both sides, e.g. for non-deliverable option instrument the short party needs to have high enough penalty margin to ensure that long party feels that short party won't bail out immediately if volatility moves against seller.
...
| Method                    | Description |
| :------------------------ | :-----------|
| **modfiyPauseStatus()**   | Only DTD admin can call this to pause / unpause DTD |
...
| Method                    | Description |
| :------------------------ | :-----------|
| **createContract()**      | Creates new contract instance to DTD. Caller is OTC instrument smart contract. Tx originator is considered to be the short party of the contract |
| **lockContract()**        | Locks the contract and makes it active. Caller is OTC instrument. Tx originator is considered to be the long party of the contract |
...
Let's assume two parties want to engage in NDF transaction, which is settled in USDC and the underlying non-convertible currency is Korean won (KRW). The NDF contract reads the rate using Chainlink oracle's stream and the counterparties agree on following terms:
...
[6] "Credit Suisse Factsheet: Non-deliverable forwards (NDFs)"
https://www.credit-suisse.com/media/assets/private-banking/docs/ch/unternehmen/kmugrossunternehmen/fx-factsheet-ndf-en.pdf

[7] "EIP 6123: Smart Derivative Contract" - Christian Friest et al
